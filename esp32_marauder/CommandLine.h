...
const char PROGMEM PASSIVE_CMD[] = "passive";
...
const char PROGMEM HELP_PORT_SCAN_CMD[] =
...
const char PROGMEM HELP_PASSIVE_CMD[] = "passive";

// WiFi attack
const char PROGMEM HELP_ATTACK_CMD[] =
    "attack -t <quiet/csa/sae/beacon [-l/-r/-a]/deauth [-c]/[-s <src mac>] [-d "
    "<dst mac>]/probe/rickroll/badmsg [-c]/sleep [-c]>";
...
private:
  ...
  void printLargeString(const char* p);

public:
  CommandLine();

  bool menu_mode;
  int current_menu_layer;
  int custom_text_target_count = -1;
  int custom_text_current_count = -1;
  int menu_input_mode = -1;
  int menu_input_step = -1;
  int menu_input_target_index = -1;
  String menu_input_buffer = "";

  void RunSetup();
  void printMainMenu();
  void printMenuLayer(int layer);
